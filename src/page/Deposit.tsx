        ...
        <div className="bg-[#877661]">
            ...
            <div className="fixed bottom-0  max-w-[38rem] w-full bg-[#877661] p-[16px] ">
                <button disabled={loading} onClick={() => {
                    return currentAddress ? onCopyAddress(currentAddress) : handleLogin()
                }} className="w-full py-[10px] rounded-[9999px] text-[#fff] font-bold text-[1.4rem] relative bg-transparent cursor-pointer border-2 border-solid border-#252525 overflow-hidden rounded-full text-#333 transition-all duration-500 ease-in-out group hover:bg-[#333] hover:text-white">
                    {loading ? <span className="z-10 font-semibold tracking-wider relative z-10">Loading...</span> : <><span className="z-10 font-semibold tracking-wider relative z-10">{currentAddress ? 'COPY ADDRESS' : 'LOGIN'}</span>
